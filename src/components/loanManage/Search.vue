				...
				<div class="pad20">
					<div class="custDistribution">
						...
						<el-form-item
						    ...
						    <el-select v-model="search.orderState" placeholder="请选择" @change='changeOrderState' clearable>
						    	<el-option v-for = '(val,ind) in productState1' :label='"M" + val' :value="val" :key='ind'></el-option>
						    </el-select>
						    --
						    <el-select v-model="search.orderStateEnd" placeholder="请选择" clearable>
						    	<el-option v-for = '(val,ind) in productState2' :label='"M" + val' :value="val" :key='ind'></el-option>
						    </el-select>
						</el-form-item>
					</div>
					<div class="pre">
...
export default {
  ...
  methods: {
    changeOrderState(val) {
      ...
      if (val) {
        let index = this.productState1.indexOf(val);
        this.productState2 = this.productState1.slice(
          index,
          this.productState1.length
        );
        this.search.orderStateEnd = "";
      }
      //			console.log(val,index,777777)
    },
      ...
      api.queryPageDictionaryDetail(pararms).then(res => {
        if (res.data.data) {
          for (var i = 1; i <= Number(res.data.data[0].price); i++) {
            this.productState1.push(i);
          }
          //  				console.log(this.productState1,666666666)
        }
      });
    },
    ...
    changeProductList() {
      ...
      this.productList_s = []
      if (!this.search.productList) return
      api
    ...
    searchFn() {
      ...
      if (Number(this.search.overDateEnd) < Number(this.search.overDateStart)) {
        this.$message({
          message:
            "输入的" +
            this.arrName[this.permission.typeNameIndex - 1] +
            "起点的天数不能大于终点的天数",
          type: "warning"
        });
      } else {
