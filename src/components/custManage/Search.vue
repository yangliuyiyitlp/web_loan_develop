 ...
 <div>
 	<div class="search borBot1px">
		<el-form :inline="true" :model="search" class="demo-form-inline" v-if = 'true'>
		  ...
		  <el-form-item  v-if='permission.showAllPararms' style='margin-bottom: 15px;'>
		  ...
		  <el-form-item v-show="hasPermission('assigningCustomers')">
		   	 <el-button type="success" :loading="loadingCustDistribution" @click='CustDistribution'>客户分配</el-button>
	...
	<!--搜索条件中的组织架构弹框--END-->
	<!--客户分配组织架构弹框--START-->
	<el-dialog title="客户分配" center width='800px' :visible.sync="CustDistributionDialog" :close-on-click-modal = 'false'>
		<!--<el-row  type="flex" style='margin-top: -25px;'>-->
			<!--<el-col :span="11" >-->
				...
				<!--<div class="v-tree-wrap" style="border: 1px solid #dcdfe6;margin-top: 10px;border-radius: 5px;    height: 220px;overflow: auto;">-->
					<!--&lt;!&ndash;<Tree :arrData='data' :idArr='idArr' ref='tree' @handleNodeClick='handleNodeClickCustDis'></Tree>&ndash;&gt;-->
...
export default {
  ...
  mounted() {
    ...
    //		console.log(this.permission.showRegType)
  },
  created() {},
  methods: {
    hasPermission(data) {
      if (
        ...
        this.permissionList.includes(data)
      ) {
        return true;
      }
      return false;
    ...
    CustDistribution() {
      ...
      this.loadingCustDistribution = true
      if (this.multipleSelectionIdList.length == 0) {
        ...
        setTimeout(()=>{
        	this.loadingCustDistribution = false
        },1500)
        return;
      }
      ...
      //
      //         this.$emit('CustDistributionFn',this.search)
      // api.queryCustDistributionByDeptId(pararms).then(res => { // 获取客户分配
